...
class MailerService:
    ...
    def __init__(self, config: dict):
        ...
        if not self.host or not self.login:
            logger.warning("MailerService initialized with incomplete SMTP config.")
    ...
    def send_digest(self, memes: List[Dict], subject: str = "Подборка мемов МемоСбор 🤡") -> bool:
        ...
        if not sections:
            logger.warning("No valid memes to include in the digest after grouping.")
            return False
